...
export class PasswordResetRepository {
  ...
  async createPasswordReset(userId: string, token: string, expiryTimeInMinutes: number = 30): Promise<PasswordReset> {
  ...
  async markTokenAsUsed(token: string): Promise<boolean> {
    ...
    const result = await this.db.prepare(
      `UPDATE "PasswordReset" SET used = 1, updatedAt = ? WHERE token = ?`
    )
    .bind(now, token)
    .run();
  ...
  async invalidateUserTokens(userId: string): Promise<boolean> {
    const now = Math.floor(Date.now() / 1000);
    const result = await this.db.prepare(
      `UPDATE "PasswordReset" SET used = 1, updatedAt = ? WHERE userId = ? AND used = 0`
    )
    .bind(now, userId)
    .run();
