...
export const createSubscription = async (c: Context<{ Bindings: Env }>) => {
  try {
    ...
    const subscription = await container.subscriptionService.createSubscription(userId, parseResult.data);
...
export const getSubscriptionById = async (c: Context<{ Bindings: Env }>) => {
  try {
    ...
    const container = getSubscriptionsContainer(c.env);
    const subscription = await container.subscriptionService.getSubscriptionById(subscriptionId, userId);
    
    if (!subscription) {
      return formatError(c, `Subscription with id ${subscriptionId} not found`, 'ResourceNotFound', 404);
    }
...
export const cancelSubscription = async (c: Context<{ Bindings: Env }>) => {
  try {
    ...
    const container = getSubscriptionsContainer(c.env);
    const subscription = await container.subscriptionService.cancelSubscription(subscriptionId, userId);
