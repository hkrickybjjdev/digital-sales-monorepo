...
export class RegistrationService implements IRegistrationService {
  ...
  async createRegistration(shortId: string, request: CreateRegistrationRequest): Promise<{ registration: Registration | null; error?: string }> {
    // Get page by shortId
  ...
  async getRegistrations(pageId: string, userId: string, limit = 100, offset = 0): Promise<{
  ...
  async exportRegistrationsAsCsv(pageId: string, userId: string): Promise<string> {
    ...
    const rows = registrations.map(registration => {
      ...
      customFieldsArray.forEach(key => {
        const value = registration.customFields && registration.customFields[key] 
