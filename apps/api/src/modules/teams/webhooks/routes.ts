...
import { handleUserCreated, handleUserUpdated, handleUserDeleted } from './authWebhookHandlers';
...
export function createWebhookRouter(): Hono<{ Bindings: Env }> {
  ...
  router.get('/', c => {
    return c.json({
      ...
      webhooks: [
        {
          path: '/auth/user-created',
          method: 'POST',
          description: 'Handle user creation events from auth module',
        },
        {
          path: '/auth/user-updated',
          method: 'POST',
          description: 'Handle user update events from auth module',
        },
        {
          path: '/auth/user-deleted',
          method: 'POST',
          description: 'Handle user deletion events from auth module',
        },
      ],
    });
  });
